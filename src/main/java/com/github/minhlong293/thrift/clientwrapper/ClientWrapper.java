...
public class ClientWrapper<T, K extends T> extends BaseClient {
    ...
    private final Supplier<TTransport> transportSupplier;
    private final Function<TTransport,K> clientSupplier;
    private AtomicBoolean isConnected = new AtomicBoolean(false);

    /**
     * @param transportSupplier
     * @param clientSupplier Client supplier function
     * @param iFaceClazz the Iface class generated by Thrift (Iface.class)
     */
    public ClientWrapper(Supplier<TTransport> transportSupplier, Function<TTransport, K> clientSupplier, Class iFaceClazz) {
    ...
    public void connect() throws TException {
        synchronized (this) {
            if (!this.isConnected()) {
                this.transport = this.transportSupplier.get();
                if (this.transport == null) throw new TTransportException("Transport is null");
                if (!this.transport.isOpen())
                    this.transport.open();
                this.clientObject = this.clientSupplier.apply(transport);
                if (this.clientObject == null) throw new TTransportException("Client is null");
